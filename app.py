...
if st.session_state.display_list:
    ...
    if st.button("✨ توليد الجدول النهائي ✨"):
        ...
        if num_days > 0 and total_chapters > 0:
            ...
            st.dataframe(
                df_final,
                column_config={"تنبيه مشترك 🔔": st.column_config.LinkColumn("اضغط للتنبيه 🤝")},
                hide_index=True,
            )
            
            buffer = io.BytesIO()
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                df_final.drop(columns=["تنبيه مشترك 🔔"]).to_excel(writer, index=False, sheet_name='Plan')
