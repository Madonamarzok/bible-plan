        ...
        if selected_books:
            ...
            if num_days > 0:
                ...
                st.download_button(
                    ...
                    file_name=f"My_Bible_Plan_{start_date}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
                
                st.info("💡 نصيحة: تقدري تفتحي ملف الـ اكسيل وتضيفي المواعيد لتقويم موبايلك عشان يجيلك تنبيهات يومية.")
                st.balloons()
            else:
                st.warning("⚠️ تاريخ النهاية لازم يكون بعد تاريخ البداية!")
        else:
            st.warning("⚠️ من فضلك اختاري سفر واحد على الأقل.")import streamlit as st
import pandas as pd
from datetime import datetime, timedelta
import io
    ...
    if st.button("توليد الجدول ✨"):
        if selected_books:
            ...
            if num_days > 0:
                ...
                st.info("💡 نصيحة: تقدري تفتحي ملف الـ Excel وتضيفي المواعيد لتقويم موبايلك عشان يجيلك تنبيهات يومية.")
                st.balloons()
            else:
                st.warning("⚠️ تاريخ النهاية لازم يكون بعد تاريخ البداية!")
        else:
